...
`scripts/setup_evaluation.sh` synchronizes that environment. It does not change
...
`scripts/prepare_corpus.py` materializes v3 from its reviewed, hash-verified source
images and records Pillow/script/image/annotation hashes. The original v2
preparation implementation is preserved as `scripts/prepare_corpus_v2.py` for
...
/home/amazi/falcon-ocr-evaluation/.venv/bin/python \
  scripts/run_official_evaluation.py \
...
```bash
python scripts/compare_official_evaluation.py \
  --manifest reference/corpus-v3-evaluation-lock.json \
