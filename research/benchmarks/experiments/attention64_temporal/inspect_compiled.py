"""Inspect one frozen temporal attention head; no builds/model/timing execution."""
import argparse
import json
...
NAME = 'falcon_ocr::kernels::temporal64::temporal_head'
PDBUTIL = Path('C:/Program Files/LLVM/bin/llvm-pdbutil.exe')
OBJDUMP = Path('C:/Program Files/LLVM/bin/llvm-objdump.exe')
UTILITY_SOURCE = 'research/benchmarks/experiments/gemv_pair/inspect_compiled.py'
UTILITY_SOURCE_SHA = 'bf17bc9cafe1ba2a0ce953a2e647ad2c76096f3bac66e7bcc7485e80eeac7dfa'
...
def summarize_instructions(instructions):
    ...
    return {'calls': calls, 'indirect_call_or_tail_transfers_among_parsed_instructions': indirect,
            ...
            'split_key_reduction_equivalence_from_disassembly': 'not_automatically_claimed'}
...
def inputs(args):
    ...
    need(prep['kind'] == 'attention64-temporal-preparation-v1'
         and build['kind'] == 'attention64-temporal-build-v1', 'Wrong experiment receipts')
    ...
    need(ops['tests'] == sorted(TEST_NAMES) and ops['selected_test_count'] == len(TEST_NAMES)
    ...
    for group in ops['groups']:
        ...
        need(group['passed'] is True and group['exit_code'] == 0
        ...
        need(f'{len(expected)} passed; 0 failed' in log
             and all(f'test {name} ... ok' in log for name in expected), 'Missing actual operator completions')
...
def main():
    ...
    benchmark_dir = directory / 'benchmark-build'
    output = benchmark_dir / 'compiled-temporal-head'
    output.mkdir(exist_ok=False)
    report = {'kind': 'attention64-temporal-compiled-head-v1', 'status': 'failed',
              ...
              'copied_utility_provenance': {'path': UTILITY_SOURCE, 'sha256': UTILITY_SOURCE_SHA,
                  ...
                  'change': 'Descriptive reduction field renamed for split-key inspection; instruction claims scoped to parsed lines and reduction candidates.'},
              ...
              'limitations': [
                  ...
                  'Opcode counts do not prove split-load/FMA dataflow or absence of a reconstructed key; manual review is required.',
    ...
    try:
        ...
        if len(matches) != 1:
            ...
            save(output, 'temporal-head-symbol.txt', symbol + '\n')
            ...
            save(output, 'temporal-head-disassembly.txt', disassembly)
            instructions = []
            ...
            report.update(status='inspected_pending_dataflow_review', section=section,
                          ...
                          split_prefix_loads='unresolved_requires_register_dataflow_review',
                          absence_of_key_reconstruction='unresolved_requires_register_dataflow_review',
