...
from contract import (ROOT, RUNTIME, STAGES, CONTROL_STAGES, PINS,
...
def compare(args):
    ...
    require(execution["kind"] == "fp32-crossover-layer7-execution-v1"
    ...
    for arrays, branch, role in [(cpu_arrays, "cpu_state", "cpu_trace"),
                                 ...
                                 (gpu_arrays, "gpu_state", "gpu_trace")]:
        original = originals[role]
        want = original[saved_name(original, "layer.6.hidden")]
    ...
    fixed = (112, 249)
    r, c, g = (float(arrays[key][fixed]) for arrays, key in (
        (cpu_arrays, "cpu_state.layer.7.hidden"), (gpu_arrays, "cpu_state.layer.7.hidden"),
        (gpu_arrays, "gpu_state.layer.7.hidden")))
    total, engine, incoming = r - g, r - c, c - g
    endpoint = {"stage": "layer.7.hidden", "coordinate": list(fixed), "rust_cpu": r,
                "gpu_cpu": c, "gpu_gpu": g, "original_rust_minus_gpu": total,
                "engine_difference_on_cpu_entry_state": engine,
                "incoming_state_difference_through_gpu": incoming,
                "signed_incoming_fraction": None if total == 0 else incoming / total,
                "signed_engine_fraction": None if total == 0 else engine / total,
                "fp64_decomposition_residual": total - (engine + incoming)}
    validate_plan(plan)
    require(all(sha(p) == h for p, h in bound.items()), "Input/source/artifact changed during comparison")
    report = {"kind": "fp32-frozen-state-crossover-layer7-analysis-v1", "status": "validated_decomposition_only",
