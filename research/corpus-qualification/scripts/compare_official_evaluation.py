...
def validate_prepared(root, manifest_path, runtime, require_audit=True):
    ...
    if require_audit:
        ...
        require(audit.get("requirements_sha256") == sha(pathlib.Path(__file__).resolve().parents[3] / "requirements/evaluation-resolved.txt"), "Execution requirements lock differs")
...
def compare(gpu, cpu, manifest_path):
    ...
    return {"schema_version": 2, "status": "complete", "scope": f"Official v1_5 component evaluation of all {len(pages['gpu'])} selected pages in {pathlib.Path(manifest_path).name}; no Overall score.",
            ...
            "evaluator_requirements_sha256": sha(pathlib.Path(__file__).resolve().parents[3] / "requirements/evaluation-resolved.txt"),
