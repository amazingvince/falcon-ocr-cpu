...
TEST = "model::fp32_crossover::capture_cpu_state_crossover"
...
def source_paths():
    ...
    extras += [ROOT / "experiments/fp32_crossover" / n for n in EXPERIMENT_SOURCES]
...
def prepare(args):
    ...
    plan = {"kind": KIND, "inputs": inputs, "source_sha256": source_hashes,
            ...
            "scope": "One layer8/state crossover followed by layer9 RMS/QKV/V; controls precede interpretation.",
    ...
    require("mod fp32_crossover;" not in original_model, "Diagnostic already installed")
    (project / "src/model.rs").write_text(original_model + MODULE, encoding="utf-8", newline="\n")
    shutil.copyfile(ROOT / "research/gpu-reference/experiments/fp32_crossover/rust_arm.rs", project / "src/fp32_crossover_arm.rs")
    (project / "src/fp32_crossover_forward.rs").write_text(adapter, encoding="utf-8", newline="\n")
    isolated = inventory(project)
    changed = {n for n, h in isolated.items() if source_hashes.get(n) != h}
    require(changed == {"src/model.rs", "src/fp32_crossover_arm.rs", "src/fp32_crossover_forward.rs"},
    ...
    prep = {"kind": "fp32-crossover-preparation-v1", "status": "prepared_not_executed",
...
def load_prepared(args):
    ...
    require(prep["kind"] == "fp32-crossover-preparation-v1" and prep["selected_test"] == TEST,
            "Wrong preparation")
...
def build(args):
    ...
    result = {**start, "kind": "fp32-crossover-build-v1", "status": "built_not_executed",
...
def run(args):
    ...
    require(build_record["kind"] == "fp32-crossover-build-v1"
    ...
    write_new(directory / "execution.json", {"kind": "fp32-crossover-execution-v1", "status": "cpu_control_exact",
