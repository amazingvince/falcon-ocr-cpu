...
#
#   bash tools/m4_check.sh            # full run (~30-60 min)
#   QUICK=1 bash tools/m4_check.sh    # build + tests + one short bench
#
...
# Windows host (see docs/PORTABILITY.md).
set -euo pipefail
...
python3 research/phase4-hillclimb/attempt3/make_manifest.py "$IMG" --sibling-ground-truth --output "$OUT/cases.json"
python3 research/phase4-hillclimb/attempt3/bench.py --binary target/release/falcon-ocr-eval --model artifacts/model \
