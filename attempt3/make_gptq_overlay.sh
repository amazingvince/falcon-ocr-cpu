...
#      pages (attempt3/gptq-calibration-pages.txt), running the FP32 model.
#   2. GPTQ-quantize each body matrix with those Grams (attempt3/w8_variants.py).
# About 10 min for step 1 and 11 min for step 2 on a 16-core desktop.
...
python attempt3/w8_variants.py --output "$out" --gram-dir "$gram" --method gptq --group 64
sha256sum "$out"
