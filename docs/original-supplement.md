...
`scripts/generate_original_supplement.py` uses pinned Pillow 12.3.0, FreeType 2.14.3, RAQM 0.10.5, HarfBuzz 14.2.1 and FriBidi 1.0.13, with two hashed DejaVu font files already installed in the reference Linux environment. Arabic/Hebrew are shaped as right-to-left logical strings. Missing-glyph sentinels and text bounds are checked during generation. The Bitstream Vera/DejaVu notice is preserved beside the images; font binaries are not included in the fixture artifacts.
...
The earlier CPU-only report retains its original pending-GPU status. Seven regression checks in `tests/test_original_supplement_comparison.py` cover blank denominators/markers, normalization, table projection, edit distances and incomplete/failing parity gates.
