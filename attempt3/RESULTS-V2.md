...
- **Harnesses:**
  - Calibration: one process per profile over all pages, compared with `attempt3/compare_profiles.py`. Raw data is in `artifacts/phase4/checks/`.
  - Journal A/B: `attempt3/ab.py`, interleaved fresh processes with `FALCON_OCR_PHASES=1`. Raw data is in `artifacts/phase4/ab-stage5/`.
  - Loop-stop simulation: `attempt3/loop_stop_sim.py`.
