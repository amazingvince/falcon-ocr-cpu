//! Included only inside model::fp32_crossover_layer7 in an isolated test-only copy.
use super::*;
use crate::trace::TensorTrace;
use std::collections::BTreeMap;
use std::path::PathBuf;

include!("fp32_crossover_layer7_forward.rs");
...
fn capture_cpu_state_crossover() -> Result<()> {
    ...
    ensure!(plan["kind"] == "fp32-frozen-state-crossover-layer7-v1", "plan kind");
    ensure!(plan["runtime"]["rows"] == 144 && plan["runtime"]["layer"] == 7
    ...
    let mut hidden = f32_tensor(&cpu, &saved_name(&cpu, "layer.6.hidden")?, &[144,768])?;
    ...
    let controls = ["layer.7.q","layer.7.k","layer.7.v","layer.7.attention","layer.7.hidden"]
    ...
    let report = serde_json::json!({"kind":"fp32-crossover-layer7-rust-arm-v1",
        ...
        "source_sha256":{"arm":digest(include_bytes!("fp32_crossover_layer7_arm.rs")),
            "adapter":digest(include_bytes!("fp32_crossover_layer7_forward.rs")),"kernels":digest(include_bytes!("kernels.rs"))},
