...
fn head_prefix_full_model_qualification() -> Result<()> {
    let output =
        PathBuf::from(std::env::var_os("FOCR_HEAD_PREFIX_MODEL_OUTPUT").context("set output file")?);
    ensure!(!output.exists(), "preserve existing run output");
    let layout_name = std::env::var("FOCR_HEAD_PREFIX_MODEL_LAYOUT")?;
    ensure!(
        ["expanded", "compact", "head_contiguous_prefix"].contains(&layout_name.as_str()),
        "unknown layout"
    );
    // The same source compiles against the genuine production crate; that crate
    // rejects head_contiguous_prefix. No cfg override or production control patch.
