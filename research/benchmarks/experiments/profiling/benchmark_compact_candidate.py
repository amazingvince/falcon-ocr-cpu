#!/usr/bin/env python3
"""One combined fixed-width/compact-cache candidate between compact CPU controls."""
import argparse
import hashlib
import json
...
def validate(plan_path, expected_sha):
    ...
    rb.require(plan["kind"] == "single-page-compact-candidate-bracket-v1", "Wrong plan kind")
...
def prepare(args):
    ...
    plan = {"kind": "single-page-compact-candidate-bracket-v1", "created_utc": rb.utc(),
            ...
            "jobs": JOBS,
            "limitations": ["One full page, one candidate, compact cache only; batch one uses the single-page path; no batch/platform promotion.",
