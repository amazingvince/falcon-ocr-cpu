"""Inspect one frozen head-contiguous prefix attention head; no builds/model/timing.

Execution-only helper: its source identity is bound separately at inspection
startup/end, without modifying the frozen preparation or build manifest.
"""
import argparse
import json
...
from patch import TEST_NAMES, TEST_FILTERS, ADDED_SOURCE_NAMES

NAME = 'falcon_ocr::kernels::head_prefix64::prefix_head'
PDBUTIL = Path('C:/Program Files/LLVM/bin/llvm-pdbutil.exe')
OBJDUMP = Path('C:/Program Files/LLVM/bin/llvm-objdump.exe')
UTILITY_SOURCE = 'experiments/attention64_temporal/inspect_compiled.py'
...
def summarize_instructions(instructions):
    ...
    return {'calls': calls, 'indirect_call_or_tail_transfers_among_parsed_instructions': indirect,
            ...
            'prefix_address_equivalence_from_disassembly': 'not_automatically_claimed'}


def direct_call_targets(instructions, symbols, publics, base, sections):
    """Join numeric direct-call targets to exact PDB addresses, never nearest names."""
    by_address = {}
    for kind, source in [('procedure', symbols), ('public', publics)]:
        pattern = (r'S_[LG]PROC32(?:_ID)?[^\n]*`([^`]+)`\n([^\n]+)'
                   if kind == 'procedure' else r'S_PUB32[^\n]*`([^`]+)`\n([^\n]+)')
        for match in re.finditer(pattern, source):
            address = re.search(r'addr = (\d+):(\d+)', match[2])
            if address is None:
                continue
            section, offset = map(int, address.groups())
            if not (1 <= section <= len(sections)):
                continue
            rva, rawsize, _, _ = sections[section - 1]
            if not (0 <= offset < rawsize):
                continue
            target = base + rva + offset
            by_address.setdefault(target, []).append({
                'kind': kind, 'name': match[1], 'section': section,
                'section_offset_decimal': offset, 'record': match.group()})
    result = []
    for address, mnemonic, operand in instructions:
        if not mnemonic.startswith('call') or operand.lstrip().startswith('*'):
            continue
        target = re.match(r'0x([0-9a-fA-F]+)\b', operand)
        if target is None:
            result.append({'call_address': hex(address), 'operand': operand,
                           'status': 'unresolved_non_numeric_direct_target'})
            continue
        target_address = int(target[1], 16)
        records = by_address.get(target_address, [])
        names = sorted({r['name'] for r in records})
        result.append({'call_address': hex(address), 'target_address': hex(target_address),
                       'target_rva': hex(target_address - base), 'operand': operand,
                       'status': 'exact_unique_name' if len(names) == 1 else
                           'exact_address_aliases' if names else 'unresolved_no_exact_pdb_address',
                       'exact_names': names, 'pdb_records': records})
    return result
...
def inputs(args):
    ...
    need(prep['kind'] == 'head-contiguous-prefix-preparation-v1'
         and build['kind'] == 'head-contiguous-prefix-build-v1', 'Wrong experiment receipts')
    need(build['status'] == 'built_not_executed'
         and build['preparation_sha256'] == args.preparation_sha256, 'Build/preparation mismatch')
    need(ops['status'] == 'passed' and ops['source_closure'] is True
    ...
    need(len(TEST_NAMES) == len(set(TEST_NAMES)) == 12
         and ops['tests'] == sorted(TEST_NAMES) and ops['selected_test_count'] == len(TEST_NAMES)
         and prep['selected_tests'] == list(TEST_NAMES)
         and prep['selected_test_count'] == len(TEST_NAMES)
         and prep['test_filters'] == list(TEST_FILTERS), 'Exact operator inventory differs')
    ...
    need([g['filter'] for g in ops['groups']] == list(TEST_FILTERS), 'Operator filter inventory differs')
    seen = set()
    for group in ops['groups']:
        expected = sorted(n for n in TEST_NAMES if group['filter'] in n)
        need(expected and group['passed'] is True and group['exit_code'] == 0
             and group['tests'] == expected and not (seen & set(expected)), 'Operator group incomplete')
        need(group['command'] == [str(operator_path), group['filter'], '--show-output',
                                 '--test-threads', '1'], 'Wrong operator command')
        seen.update(expected)
        path = (directory / group['log']).resolve()
        need(path.parent == directory, 'Unexpected operator log path')
        raw = path.read_bytes()
        need(digest(raw) == group['log_sha256'], 'Operator log changed')
        log = raw.decode('utf-8')
        need(f'{len(expected)} passed; 0 failed' in log
             and all(log.splitlines().count(f'test {name} ... ok') == 1 for name in expected),
             'Missing/duplicate actual operator completions')
        bound[path] = group['log_sha256']
    need(seen == set(TEST_NAMES), 'Missing operator tests')
    ...
    names = set(control['source_sha256']) | set(ADDED_SOURCE_NAMES)
...
def main():
    ...
    benchmark_dir = directory / 'benchmark-build'
    output = benchmark_dir / 'compiled-prefix-head'
    output.mkdir(exist_ok=False)
    report = {'kind': 'head-contiguous-prefix-compiled-head-v1', 'status': 'failed',
              ...
              'copied_utility_provenance': {'path': UTILITY_SOURCE, 'sha256': UTILITY_SOURCE_SHA,
                  ...
                  'change': 'Head-prefix build/operator validation and exact function symbol; direct-call names require exact target addresses, with parsed-line and dataflow limitations retained.'},
              ...
              'limitations': [
                  ...
                  'Opcode counts do not prove head-contiguous prefix K/V addressing or unchanged generated-cache strides; manual dataflow review is required.',
                  ...
                  'Direct-call names require exact PDB address equality; aliases/unresolved targets remain explicit, and call targets are not disassembled.',
    ...
    try:
        ...
        if len(matches) != 1:
            ...
            save(output, 'prefix-head-symbol.txt', symbol + '\n')
            ...
            save(output, 'prefix-head-disassembly.txt', disassembly)
            instructions = []
            ...
            targets = direct_call_targets(instructions, symbols, publics, base, sections)
            save(output, 'direct-call-targets.json', json.dumps(targets, indent=2) + '\n')
            report.update(status='inspected_pending_dataflow_review', section=section,
                          ...
                          head_contiguous_prefix_loads='unresolved_requires_register_dataflow_review',
                          generated_cache_addressing='unresolved_requires_register_dataflow_review',
                          direct_call_target_resolution=targets,
