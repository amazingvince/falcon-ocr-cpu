...
fn read(tensors: &SafeTensors<'_>, name: &str) -> Result<Vec<bf16>> {
    ...
    Ok(tensor.data().chunks_exact(2).map(|b| bf16::from_bits(u16::from_le_bytes(b.try_into().unwrap()))).collect())
}
fn compare(expected: &[bf16], actual: &[bf16]) -> Value {
    ...
    let ordered = |value: bf16| { let bits=value.to_bits(); if bits & 0x8000 != 0 { !bits } else { bits ^ 0x8000 } };
    for (index, (a, b)) in expected.iter().zip(actual).enumerate() {
        ...
        if a.is_finite() && b.is_finite() {
            ...
            if ulp > max_ulp { max_ulp = ulp; worst_index = Some(index); }
        }
    }
...
fn main() -> Result<()> {
    ...
    let operators = pool.install(|| -> Result<Vec<Value>> {
        ...
        for (name,width) in [("rms.width768.actual",768),("rms.width64.actual",64),
                             ("rms.width768.small",768),("rms.width64.small",64)] {
        ...
        for name in ["prefill.final_norm","decode.2.final_norm","decode.6.final_norm"] {
            let input = read(&affine_tensors,&format!("{name}.input"))?;
            let expected = read(&affine_tensors,&format!("{name}.expected"))?;
            let mut actual = vec![bf16::ZERO; expected.len()];
            bf16_ops::rms_norm(&input,&mut actual,weight.len(),1e-5,Some(&weight),&mut scratch);
            reports.push(json!({"operator":name,"comparison":compare(&expected,&actual)}));
        }
        Ok(reports)
    })?;
