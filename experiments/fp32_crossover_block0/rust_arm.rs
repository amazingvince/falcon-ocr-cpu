//! Included only inside model::fp32_crossover_block0 in an isolated test-only copy.
use super::*;
use crate::trace::TensorTrace;
use std::collections::BTreeMap;
use std::path::PathBuf;

include!("fp32_crossover_block0_forward.rs");
...
fn capture_cpu_state_crossover() -> Result<()> {
    ...
    ensure!(plan["kind"] == "fp32-frozen-state-crossover-block0-v1", "plan kind");
    ensure!(plan["runtime"]["rows"] == 144 && plan["runtime"]["layer"] == 0
        && plan["runtime"]["rust_threads"] == 4
        && plan["runtime"]["rust_backend"] == "avx2"
        && plan["runtime"]["rust_toolchain"] == "1.92.0"
        && plan["original_state_key"] == "embedding", "fixed runtime");
    ...
    let mut hidden = f32_tensor(&cpu, &saved_name(&cpu, "embedding")?, &[144,768])?;
    ...
    ensure!(trace.tensors.len() == expected_stages.len() && expected_stages.len() == 14, "capture inventory");
    ...
    let captured_input = trace.tensors.get("cpu_state.input").context("missing original entry capture")?;
    let input_bit_exact = digest(bytemuck::cast_slice(&captured_input.1)) == original_input_sha;
    ensure!(input_bit_exact, "captured full embedding differs from original CPU entry");
    let controls = ["layer.0.q","layer.0.k","layer.0.v","layer.0.attention","layer.0.hidden"]
    ...
    let report = serde_json::json!({"kind":"fp32-crossover-block0-rust-arm-v1",
        ...
        "input_bit_exact":input_bit_exact,"native_block0_calls":native_block0_calls,
        ...
        "source_sha256":{"arm":digest(include_bytes!("fp32_crossover_block0_arm.rs")),
            "adapter":digest(include_bytes!("fp32_crossover_block0_forward.rs")),"kernels":digest(include_bytes!("kernels.rs"))},
