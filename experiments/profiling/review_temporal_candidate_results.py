"""Explicit temporal-prefix B1 saved-result review after parent timing release.

The compact control is the accepted combined-attention build. The candidate
uses the distinct temporal_candidate report / temporal-candidate CLI spelling.
...
PROTOCOL_NAME = "experiments/profiling/benchmark_temporal_candidate.py"
PROTOCOL_SHA = "6b8ec9382e674d913a8684f5942f9f0533d12c5c08c7b1e3e3fa4cf24787de06"
...
LAYOUTS = {"control": {"cli": "compact", "report": "compact"},
           "candidate": {"cli": "temporal-candidate", "report": "temporal_candidate"}}
bound = {}
checks = 0
...
def load_protocol(plan):
    ...
    helper = ROOT / "scripts/realistic_benchmark.py"
    payload(helper, plan["files_sha256"][str(helper)])
    spec = importlib.util.spec_from_file_location("temporal_review_protocol", path)
...
def main(args):
    ...
    check(plan["kind"] == "single-page-temporal-candidate-bracket-v1" and plan["output_directory"] == str(FOLDER), "Exact temporal plan scope")
    ...
    with zipfile.ZipFile(io.BytesIO(payload(protocol_path))) as archive:
        check(len(archive.namelist()) == 2 and set(archive.namelist()) == {PROTOCOL_NAME, "scripts/realistic_benchmark.py"}, "Protocol archive inventory")
    ...
    for job, saved in zip(plan["jobs"], comparison["reports"]):
        ...
        rows.append({"name": name, "cache_layout": LAYOUTS[job["build"]]["report"], "median_ms": med, "samples_ms": saved["samples_ms"], "stage_medians_ms": stages, "pid": execution["pid"]})
    ...
    receipt={"schema_version":1,"kind":"windows-temporal-candidate-fullpage-independent-review-v1","status":"passed_saved_result_review","plan_sha256":PLAN_SHA,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=Path,
                        default=ROOT / "reference/benchmarks/windows-temporal-candidate-fullpage-review-v1.json")
