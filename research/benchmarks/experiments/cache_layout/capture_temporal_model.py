...
def prepare(output):
    ...
    for path in [Path(__file__), HERE / "temporal_model_qualification.rs", HERE / "test_temporal_model_capture.py"]:
        inputs[path.relative_to(ROOT).as_posix()] = sha(path)
    inputs["tools/build_windows.ps1"] = native["original_source_sha256"]["tools/build_windows.ps1"]
    ...
    with zipfile.ZipFile(output / "source.zip", "x", compression=zipfile.ZIP_DEFLATED) as archive:
        ...
        for name in [Path(__file__).relative_to(ROOT).as_posix(),
                     (HERE / "test_temporal_model_capture.py").relative_to(ROOT).as_posix(),
                     "tools/build_windows.ps1", "tests/cache_layout.rs", "tests/decode_allocations.rs", "tests/weight_layout.rs"]:
            archive.writestr("capture/" + name, checked_bytes(ROOT / name, inputs[name]))
    plan = {"schema_version": 1, "kind": "temporal-prefix-model-qualification-v1", "inference_executed": False,
...
def validate_plan(path):
    ...
    require(plan["kind"] == "temporal-prefix-model-qualification-v1" and plan["runtime"] == RUNTIME, "Changed plan options")
    ...
    with zipfile.ZipFile(output / "source.zip") as archive:
        ...
        expected.update({"capture/" + n: plan["inputs_sha256"][n] for n in [Path(__file__).relative_to(ROOT).as_posix(),
            (HERE / "test_temporal_model_capture.py").relative_to(ROOT).as_posix(), "tools/build_windows.ps1",
...
def build(path):
    ...
    output = Path(plan["output_directory"])
    env = dict(os.environ, CARGO_TARGET_DIR=str(ROOT / "target"), CUDA_VISIBLE_DEVICES="-1", CARGO_BUILD_JOBS="2")
        ...
        for label in ["control", "candidate"]:
            ...
            for line in log.read_text(encoding="utf-8").splitlines():
                try:
                    ...
                    continue
                if item.get("reason") == "compiler-artifact" and item.get("target", {}).get("name") == TEST_NAME and item.get("profile", {}).get("test") and item.get("executable"):
                    emitted.add(item["executable"])
            require(len(emitted) == 1, "Ambiguous emitted qualification executable")
            binary = output / (label + "-qualification.exe")
            shutil.copyfile(emitted.pop(), binary)
...
def run(path):
    ...
    require(set(built["projects"]) == {"control", "candidate"}, "Build inventory differs")
    ...
    try:
        for label, project, layout in JOBS:
            ...
            execution["jobs"].append({"name": label, "result_sha256": bound[result_path], "log_sha256": bound[log]})
            stable()
