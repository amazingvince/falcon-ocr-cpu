...
import traceback
import zipfile
from patch import ROOT, KERNELS_SHA, make_patch, OLD, PRIOR_PREP
...
def source_map():return {p.relative_to(ROOT).as_posix():sha(p) for p in [HERE/n for n in SOURCES]+[OLD/n for n in ['patch.py','candidate.rs','tests.rs']]+[PRIOR_PREP,ROOT/'tests/decode_allocations.rs']}
...
def prepare(args):
    ...
    record={'kind':'attention64-compact-preparation-v1','status':'prepared_not_built',
        ...
        'test_filter':FILTER,'selected_test_count':11,'model_execution_allowed':False}
...
def load(args):
    ...
    need(record['kind']=='attention64-compact-preparation-v1' and record['test_filter']==FILTER,'Wrong preparation')
...
def build(args):
    ...
    result={**start,'status':'built_not_executed','kind':'attention64-compact-build-v1','executables':executables,
...
def operators(args):
    ...
    need(len(names)==11 and all(n.startswith(FILTER) for n in names),'Wrong eleven-test inventory')
    ...
    passed=code==0 and '11 passed; 0 failed' in text and all(f'test {n} ... ok' in text for n in names)
