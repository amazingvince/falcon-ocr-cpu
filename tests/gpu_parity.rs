...
use falcon_ocr::{Backend,Model,Runner,RunnerConfig,GenerationOptions,FinishReason,kernels::Simd};
...
fn free_running_cpu_matches_gpu_smoke_tokens_and_stop() {
    ...
    let metadata:serde_json::Value=serde_json::from_reader(std::fs::File::open(reference.join("metadata.json")).unwrap()).unwrap();
    ...
    let runner=Runner::new(model.clone(),model_dir,RunnerConfig{threads:4,batch_size:1,..Default::default()}).unwrap();
    let options=GenerationOptions {max_dimension:metadata["max_dimension"].as_u64().unwrap() as u32,
        min_dimension:metadata["min_dimension"].as_u64().unwrap() as u32,
        max_new_tokens:metadata["max_new_tokens"].as_u64().unwrap() as usize};
    let result=runner.recognize_file(reference.join("canonical-rgb.png"),&options).unwrap();
    ...
    let limited=runner.recognize_file(reference.join("canonical-rgb.png"),
        &GenerationOptions{max_new_tokens:3,..options.clone()}).unwrap();
    ...
    for (backend,simd) in [(Backend::Avx2,Simd::Avx2),(Backend::Avx512,Simd::Avx512)] {
        if simd.validate().is_err(){continue;}
        let runner=Runner::new(model.clone(),model_dir,RunnerConfig{threads:1,backend,..Default::default()}).unwrap();
        let result=runner.recognize_file(reference.join("canonical-rgb.png"),&options).unwrap();
