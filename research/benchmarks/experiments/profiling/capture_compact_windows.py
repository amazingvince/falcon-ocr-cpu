...
PIN = {
    'build': '68b1667cb7fed0d5470ec8b9153e87dee7198787f70667e71f807be9f8cae7f0',
    'binary': '8de750766ea4026303f7d3b8f0c1ce91da67b12b4af293245cf24faa98d3a3ac',
    'source_archive': 'f39d1f4641237aa82dc318d9bc14d5a28c71c292c27fdd497d39253c257ac4d3',
    'pdb': '1e7e0f0d0640a94daec5c9b990fd37d4dfdaa6c7e868360fd61f1bb137abcba4',
    'symbols_receipt': 'ea4842b0289c971a72f6e22bb0d5d10b623743376685159e34ef894444f3acf3',
    ...
    'baseline': 'de250d2ad37db564503762952fa8557c05dde24c48d2f4d659af136a7752fa91',
}
SOURCE_NAMES = ['capture_compact_windows.py', 'launch_compact_elevated.ps1',
                'analyze_compact_stacks.py', 'FalconOcrCpu.wprp',
                'COMPACT_PROFILE_ACCEPTANCE.md', 'COMPACT_PROFILE_README.md', 'NEXT-PROFILE-V1.md']
...
def command_for(folder, image):
    return [str(BUILD / 'ocr_bench.exe'), '--model', str(ROOT / 'artifacts/model'),
            '--threads', '16', '--backend', 'avx2', '--execution', 'joint',
            '--cache-layout', 'compact', '--weight-layout', 'unpacked', '--batches', '1',
...
def prepare(folder):
    ...
    symbols = json.loads(checked(BUILD / 'compiled-compact-head/symbol-receipt.json', PIN['symbols_receipt'], bindings))
    require(symbols['input_sha256'][str(BUILD / 'ocr_bench.exe')] == PIN['binary']
            and symbols['input_sha256'][str(BUILD / 'build.json')] == PIN['build']
            and symbols['pdb_sha256'] == PIN['pdb']
            and (ROOT / symbols['preserved_pdb']).resolve() == (BUILD / 'ocr_bench.pdb').resolve()
            and symbols['pdb_identity_matches_pe'] is True
            and symbols['pe_codeview']['guid'].lower() == '1797f1f9-ec73-4291-98a8-5c9c8aaf2ae1'
            and symbols['pe_codeview']['age'] == 1, 'Frozen executable/PDB pair')
    ...
    bindings[str(python)] = file_sha(python)
    ...
    plan = {'schema_version': 1, 'kind': 'falcon-ocr-owned-native-compact-cpu-profile-v1', 'created_utc': utc(),
            'output': str(folder), 'instance': instance, 'pins': PIN, 'limits': LIMITS,
            'runtime_python': str(python), 'runtime_python_sha256': bindings[str(python)],
            ...
            'recording_started': False, 'analysis_required': ['Exact ETW PID/image/start/end and unique process-name match',
                'Full actual process lifetime and three recognitions; zero available event-loss counters and truncation; >=99% attached stacks with missing counts retained; unavailable buffer loss stays unknown',
...
def benchmark_check(file):
    ...
    require(actual['os'] == 'windows' and actual['arch'] == 'x86_64', 'Native platform required')
    ...
    require(case['batch_size'] == case['active_batch_size'] == 1 and case['execution'] == 'independent_prefill_joint_decode', 'Single-page execution')
...
def run(plan_file, expected):
    ...
    require(plan.get('schema_version') == 1
            and plan.get('kind') == 'falcon-ocr-owned-native-compact-cpu-profile-v1', 'Unexpected compact plan kind/schema')
    ...
    require(Path(sys.executable).resolve() == Path(plan['runtime_python']).resolve()
            and file_sha(Path(sys.executable)) == plan['runtime_python_sha256'], 'Python runtime changed')
