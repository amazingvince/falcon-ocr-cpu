...
`scripts/build_aocl.py` now records source hashes before configuration and again
...
```powershell
python scripts/build_aocl.py --source artifacts/aocl-dlp `
  --build artifacts/aocl-dlp-build-windows-reproduction --jobs 2 `
  --cmake "C:/Program Files/CMake/bin/cmake.exe"
python scripts/capture_rust_build.py --example aocl_probe `
...
```bash
python3 scripts/build_aocl.py --source artifacts/aocl-dlp \
...
python3 scripts/capture_rust_build.py --example aocl_probe --jobs 2 \
...
```bash
python3 scripts/compare_aocl_probes.py \
