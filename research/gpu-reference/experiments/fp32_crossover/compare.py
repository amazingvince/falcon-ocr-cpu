...
def compare(args):
    ...
    require(execution["kind"] == "fp32-crossover-execution-v1"
    ...
    require(gpu["status"] == "gpu_arm_control_passed_crossover_captured"
            ...
            and gpu["native_layer8_calls"] == gpu["native_layer9_qkv_calls"] == 2
    ...
    for arrays, branch, role in [(cpu_arrays, "cpu_state", "cpu_trace"),
                                 ...
                                 (gpu_arrays, "gpu_state", "gpu_trace")]:
        original = originals[role]
        want = original[saved_name(original, "layer.7.hidden")]
    ...
    report = {"kind": "fp32-frozen-state-crossover-analysis-v1", "status": "validated_decomposition_only",
