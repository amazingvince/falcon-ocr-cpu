...
import instrument_bf16_ptx_v2 as instrument
import bf16_ptx_observer_support_v2 as support
...
class ObserverTests(unittest.TestCase):
    ...
    def test_exactly_four_approved_static_stores_removed(self):
        import instrument_bf16_ptx as original_builder
        v1, _ = original_builder.build(self.original)
        v1_lines = v1.splitlines(keepends=True)
        removed = [line for line in v1_lines if re.search(rb"st\.global\.v2\.b32 \[%obs_rd3\+(1088|40000|195648|234560)\], \{ %r2099, %obs_r7 \};", line)]
        self.assertEqual(len(removed), 4)
        self.assertEqual(b"".join(line for line in v1_lines if line not in removed), self.candidate)
        self.assertEqual(len(self.mapping["omitted_stores"]), 4)
    ...
    def test_complete_coverage_and_exact_bf16_unpack(self):
        for layer in [0,17,19]:
            ...
            self.assertEqual(report["expected_cells"], {0:6365,17:4254,19:4238}[layer])
            self.assertEqual(report["declared_unavailable_argument_cells"], {0:16,17:0,19:16}[layer])
    ...
    def test_unavailable_arguments_have_masks_and_no_inferred_values(self):
        total_unavailable = 0
        for layer in [0,17,19]:
            values, _ = support.decode_buffer(self.make_buffer(layer).tobytes(),layer)
            for p, (probe_layer,row,head) in enumerate(instrument.PROBES):
                if probe_layer != layer:
                    continue
                for step in range(4):
                    name = f"layer{layer}.row{row}.head{head}.tile{step}.exp2_argument_f32"
                    missing = list(instrument.unavailable_columns(p,step,"exp2_argument_f32"))
                    total_unavailable += len(missing)
                    self.assertEqual(np.flatnonzero(~values[name+".available"]).tolist(),missing)
                    self.assertTrue(np.all(np.isnan(values[name][missing])))
                    self.assertTrue(np.all(np.isfinite(values[name][values[name+".available"]])))
                    if missing:
                        words = self.make_buffer(layer)
                        base = instrument.HEADER_BYTES+p*instrument.PROBE_BYTES+step*instrument.STEP_BYTES
                        cell = (base+instrument.FIELDS.index("exp2_argument_f32")*instrument.FIELD_BYTES)//8
                        for mutation in ["flag", "data"]:
                            modified = words.copy().reshape(-1,2)
                            modified[cell,1 if mutation=="flag" else 0] = 1
                            with self.assertRaises(ValueError):
                                support.decode_buffer(modified.tobytes(),layer)
        self.assertEqual(total_unavailable,32)

    def test_machine_inventory_gate_function_unchanged(self):
        import ast
        files = [ROOT / "scripts/bf16_ptx_observer_support.py", ROOT / "research/bf16-graph/scripts/bf16_ptx_observer_support_v2.py"]
        functions = []
        for path in files:
            source = path.read_text(encoding="utf-8")
            function = next(node for node in ast.parse(source).body if isinstance(node,ast.FunctionDef) and node.name=="arithmetic_inventory")
