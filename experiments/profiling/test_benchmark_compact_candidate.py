...
SPEC = importlib.util.spec_from_file_location("compact_candidate_protocol", Path(__file__).with_name("benchmark_compact_candidate.py"))
...
class CandidateProtocolTests(unittest.TestCase):
    def test_real_compact_baseline_and_wrong_cache_rejection(self):
        root = bc.ROOT
        baseline = bc.rb.read(root / 'artifacts/benchmarks/fullpages-b1-window-v1/fullpages-b1-compact.json')
        workload = bc.rb.read(bc.rb.DEFAULT_WORKLOAD)
        build = bc.rb.read(root / 'artifacts/builds/ocr-bench-fullpages-v2-windows/build.json')
        signatures = bc.baseline_signatures(baseline, workload, build)
        self.assertEqual(len(signatures), 1)
        self.assertEqual(signatures[0]['output_tokens'], 1140)
        self.assertEqual(signatures[0]['finish_reason'], 'eos')
        baseline['cache_layout'] = 'expanded'
        with self.assertRaisesRegex(ValueError, 'cache_layout mismatch'):
            bc.baseline_signatures(baseline, workload, build)

    def protocol(self, folder):
        plan = {
            "kind": "single-page-compact-candidate-bracket-v1",
        ...
        required = {str(Path(bc.__file__).resolve()), str(bc.ROOT / "scripts/realistic_benchmark.py"),
