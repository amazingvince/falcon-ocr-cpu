...
cmd=[str(binary),'--model',str(ROOT/'artifacts/model'),'--threads','4','--backend','avx2',
     '--precision','fp32','--batch-size','1','--cache-layout','compact','--weight-layout','unpacked',
...
report={'kind':'attention64-compact-smoke-v1','exit_code':code,'build_sha256':build_sha,'binary_sha256':binary_sha,
...
try:
    ...
    fields=['token_ids','text','finish_reason','output_tokens','input_tokens','precision','teacher_forced',
            'width','height','backend','weight_layout','packed_weight_bytes']
    need(result['cache_layout']=='compact' and control['cache_layout']=='expanded','Wrong cache layouts')
