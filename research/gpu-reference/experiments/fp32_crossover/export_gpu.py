...
REQUIRED_SOURCES = {
    "research/gpu-reference/experiments/fp32_crossover/export_gpu.py", "research/gpu-reference/experiments/fp32_crossover/contract.py",
...
def execute(args):
    ...
    report = {"schema_version": 1, "status": "started", "plan_sha256": args.plan_sha256,
              ...
              "provenance_limits": [
                  ...
                  "This re-enters native layer8 and layer9 QKV from full saved layer7 states. Earlier-layer execution and allocator history are absent.",
    ...
    try:
        ...
        with safe_open(str(roles["gpu_trace"]), framework="pt", device="cpu") as g:
            gpu_state = saved(g, "layer.7.hidden")
        ...
        with safe_open(str(roles["cpu_trace"]), framework="pt", device="cpu") as c:
            cpu_state = saved(c, "layer.7.hidden")
...
def main():
    ...
    report = execute(args)
    print(json.dumps({k: report[k] for k in ("status", "native_layer8_calls", "native_layer9_qkv_calls", "gpu_control_passed", "crossover_executed")}))
