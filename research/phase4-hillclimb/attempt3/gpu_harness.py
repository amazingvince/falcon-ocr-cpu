...
* `import-vllm`: turn `research/vllm-serving/scripts/request_vllm_heldout.py` records (production
  vLLM outputs with top-K log-probabilities) into `score`/`sweep` references.

  python research/phase4-hillclimb/attempt3/gpu_harness.py reference --precision bf16 --pages P.txt --out DIR
  python research/phase4-hillclimb/attempt3/gpu_harness.py score --reference DIR --overlay W.safetensors --report R.json
"""
