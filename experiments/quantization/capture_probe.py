...
spec = importlib.util.spec_from_file_location("capture_rust_build", ROOT / "scripts/capture_rust_build.py")
...
def main():
    ...
    if args.run:
        ...
        with (output / "checker.log").open("x", encoding="utf-8") as log:
            subprocess.run([sys.executable, str(ROOT / "experiments/quantization/check_probe.py"), str(result), "--output", str(output / "independent-check.json")],
