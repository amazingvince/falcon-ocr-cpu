...
from contract import (ROOT, KIND, PINS, METADATA, RUNTIME, STAGES, CONTROL_STAGES, BUDGET,
                      FAILURE_CONTEXT, FROZEN_TOOLCHAIN,
                      require, sha, read_bound, resolve, write_new, validate_plan)
from source_guard import check_sources

TEST = "model::fp32_crossover_block0::capture_cpu_state_crossover"
EXPERIMENT_SOURCES = ["contract.py", "adapter.py", "rust_arm.rs", "capture.py",
                      "compare.py", "test_source.py", "export_gpu.py", "source_guard.py", "README.md", "SOURCE_REVIEW_NOTES.md"]
...
ENV_KEYS = ["CARGO_TARGET_DIR", "CARGO_HOME", "RUSTUP_HOME", "RUSTUP_TOOLCHAIN", "RUSTFLAGS", "CARGO_ENCODED_RUSTFLAGS",
...
MODULE = '\n#[cfg(test)]\n#[path = "fp32_crossover_block0_arm.rs"]\nmod fp32_crossover_block0;\n'


def source_paths():
    # This bounded inventory is evaluated only during authorized preparation.
    copied = [ROOT / n for n in ("Cargo.toml", "Cargo.lock", FROZEN_TOOLCHAIN)]
    copied += sorted((ROOT / "src").glob("*.rs"))
    copied += sorted((ROOT / "examples/support").glob("*.rs"))
    copied += sorted((ROOT / "tests/fixtures").glob("*.json"))
    extras = [ROOT / "tools/build_windows.ps1"]
    extras += [ROOT / "experiments/fp32_crossover_block0" / n for n in EXPERIMENT_SOURCES]
    extras += [ROOT / n for n in GPU_SOURCES]
    extras += [ROOT / "experiments/fp32_crossover_layer7" / n for n in ("adapter.py", "export_gpu.py")]
    return copied, sorted(set(copied + extras))


def copied_name(source):
    return "rust-toolchain.toml" if source == ROOT / FROZEN_TOOLCHAIN else source.relative_to(ROOT).as_posix()
...
def prepare(args):
    ...
    plan = {"kind": KIND, "inputs": inputs, "source_sha256": source_hashes,
            ...
            "scope": "Three native block0 evaluations from complete saved embeddings; five controls per original-state arm, no suffix or generation.",
    ...
    for source in copied:
        target = project / copied_name(source)
    ...
    require("mod fp32_crossover_block0;" not in original_model, "Diagnostic already installed")
    (project / "src/model.rs").write_text(original_model + MODULE, encoding="utf-8", newline="\n")
    shutil.copyfile(ROOT / "research/gpu-reference/experiments/fp32_crossover_block0/rust_arm.rs", project / "src/fp32_crossover_block0_arm.rs")
    (project / "src/fp32_crossover_block0_forward.rs").write_text(adapter, encoding="utf-8", newline="\n")
    isolated = inventory(project)
    copied_hashes = {copied_name(p): source_hashes[p.relative_to(ROOT).as_posix()] for p in copied}
    changed = {n for n, h in isolated.items() if copied_hashes.get(n) != h}
    require(changed == {"src/model.rs", "src/fp32_crossover_block0_arm.rs", "src/fp32_crossover_block0_forward.rs"},
    ...
    prep = {"kind": "fp32-crossover-block0-preparation-v1", "status": "prepared_not_executed",
            ...
            "copied_original_source_sha256": copied_hashes,
            "toolchain_override": {"path": FROZEN_TOOLCHAIN, "channel": "1.92.0", "live_toolchain_ignored": True},
...
def load_prepared(args):
    ...
    require(prep["kind"] == "fp32-crossover-block0-preparation-v1" and prep["selected_test"] == TEST,
            "Wrong preparation")
...
def tool_environment(target):
    environment = dict(os.environ, CARGO_TARGET_DIR=str(target), CUDA_VISIBLE_DEVICES="-1", RUSTUP_TOOLCHAIN="1.92.0")
    ...
    # The PATH executables may be rustup proxies. Bind the selected 1.92
    # compiler and cargo bytes too; never install or silently choose 1.94.
    for name in ("rustc", "cargo"):
        selected = subprocess.check_output(["rustup", "which", "--toolchain", "1.92.0", name],
                                           env=environment, text=True).strip()
        tools["toolchain_" + name] = Path(selected).resolve()
...
def build(args):
    ...
    command = ["powershell.exe", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
               str(ROOT / "tools/build_windows.ps1"), "test", "--locked", "--offline", "--release",
    ...
    require(start["rustc_version"].startswith("rustc 1.92.0 ")
            and start["cargo_version"].startswith("cargo 1.92.0 "), "Original 1.92.0 toolchain required")
    ...
    result = {**start, "kind": "fp32-crossover-block0-build-v1", "status": "built_not_executed",
...
def run(args):
    ...
    require(build_record["kind"] == "fp32-crossover-block0-build-v1"
    ...
    environment = dict(os.environ, CUDA_VISIBLE_DEVICES="-1", RUSTUP_TOOLCHAIN="1.92.0", FOCR_CROSSOVER_ROOT=str(ROOT),
    ...
    with (directory / "run.log").open("x", encoding="utf-8") as log:
        code = subprocess.run(command, cwd=ROOT, env=environment, stdout=log, stderr=subprocess.STDOUT, timeout=300).returncode
    log_bytes = (directory / "run.log").read_bytes()
    log_text = log_bytes.decode("utf-8")
    bound[directory / "run.log"] = hashlib.sha256(log_bytes).hexdigest()
    ...
    require(report["status"] == "control_exact" and report["plan_sha256"] == prep["plan_sha256"]
            and report["test_binary_sha256"] == build_record["binary_sha256"]
            and report["native_block0_calls"] == 1 and report["input_bit_exact"] is True,
            "Rust control/build mismatch")
    ...
    write_new(directory / "execution.json", {"kind": "fp32-crossover-block0-execution-v1", "status": "cpu_control_exact",
