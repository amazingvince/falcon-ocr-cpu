#!/usr/bin/env python3
"""Prepare the one reduced v2 observer for byte-pinned emitted PTX.
...
OMITTED_REGISTER = "%r2099"
OMITTED_PROBES = (0, 1, 5, 6)
OMITTED_COLUMNS = (0, 2, 4, 6)
...
def unavailable_columns(probe, step, field):
    if probe in OMITTED_PROBES and step in (0, 1) and field == "exp2_argument_f32":
...
def build(original):
    ...
    newline = "\r\n" if original.count(b"\r\n") else "\n"
    ops = parse_ops(lines)
    insertions, sites, mappings, omitted_stores = {}, [], [], []
        ...
        for p, mapping in enumerate(mappings):
            ...
            for field, registers in vectors.items():
                ...
                for slot, register_index in enumerate(mapping["register_indices"]):
                    ...
                    if label == "loop0_exp2_arguments" and field == "exp2_argument_f32" and register == OMITTED_REGISTER:
                        require(p in OMITTED_PROBES and slot == 0, "Unexpected omitted-register mapping")
                        omitted_stores.append({"probe": p, "field": field, "register": register,
                                               "offset": offset, "lane_slot": slot,
                                               "masked_steps": [0, 1], "unavailable_columns": list(OMITTED_COLUMNS)})
                        continue
    ...
    require([entry["offset"] for entry in omitted_stores] == [1088, 40000, 195648, 234560], "Exactly four approved stores must be omitted")
    metadata = {"schema_version": 1, "original_ptx_sha256": PTX_SHA,
                ...
                "observer_version": 2, "omitted_stores": omitted_stores,
                "unavailable_values_recomputed_or_inferred": False,
