...
QUALIFICATION=HERE/'head_prefix_model_qualification.rs'
QUALIFICATION_SHA='8f2f460d55f80380cbe60992f2895c27d7c01ca9746cd7d6a54f3e96daf54c40'
...
def prepare(args):
    ...
    need(TEST_NAMES and TEST_FILTERS and len(TEST_NAMES)==len(set(TEST_NAMES)),
         'Final nonempty operator inventory required before preparation')
    ...
    fixtures['tests/head_prefix_model_qualification.rs']=QUALIFICATION.read_bytes()
    ...
    record={'kind':'head-contiguous-prefix-preparation-v1','status':'prepared_not_built',
...
def load(args):
    ...
    need(record['kind']=='head-contiguous-prefix-preparation-v1' and record['test_filters']==list(TEST_FILTERS),'Wrong preparation')
...
def build(args):
    ...
    for label,flags,kind,name in [('cli',['build','--locked','--release','--bin','falcon-ocr'],['bin'],'falcon-ocr'),
        ...
        ('qualification',['test','--locked','--release','--no-run','--test','head_prefix_model_qualification'],['test'],'head_prefix_model_qualification')]:
        ...
        need(Path(row['target']['src_path']).resolve()==project/({'cli':'src/main.rs','operators':'src/lib.rs','qualification':'tests/head_prefix_model_qualification.rs'}[label]),'Wrong Cargo source')
    ...
    result={**start,'status':'built_not_executed','kind':'head-contiguous-prefix-build-v1','executables':executables,
...
def operators(args):
    ...
    need(build['kind']=='head-contiguous-prefix-build-v1' and build['preparation_sha256']==args.preparation_sha256
    ...
    for index,group in enumerate(groups):
        ...
        passed=(code==0 and f"{len(group['tests'])} passed; 0 failed" in text
                and all(text.splitlines().count(f'test {name} ... ok') == 1 for name in group['tests']))
