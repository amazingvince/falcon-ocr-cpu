...
fn mixed_batches_preserve_tokens_order_stops_and_limits() {
    ...
    let original=image::open("artifacts/reference/smoke-fp32/canonical-rgb.png").unwrap().to_rgb8();
    ...
    let options=GenerationOptions{max_dimension:256,max_new_tokens:24,..Default::default()};
    let sequential=Runner::new(model.clone(),"artifacts/model",RunnerConfig{threads:4,batch_size:1,..Default::default()}).unwrap();
    let expected=images.iter().map(|image|sequential.recognize(image,&options).unwrap()).collect::<Vec<_>>();
    assert!(expected.iter().any(|r|r.output_tokens!=expected[0].output_tokens),"fixture must exercise mixed stop lengths");
    for size in [2,4,8] {
        let runner=Runner::new(model.clone(),"artifacts/model",RunnerConfig{threads:4,batch_size:size,backend:Backend::Auto,..Default::default()}).unwrap();
        ...
        let limited=runner.recognize_batch(&input,&GenerationOptions{max_new_tokens:3,..options.clone()}).unwrap();
