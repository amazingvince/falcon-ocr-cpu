...
import traceback
import zipfile
from patch import ROOT, KERNELS_SHA, make_sources, BASELINE_BUILD_SHA, BASELINE_ARCHIVE_SHA, TEST_NAMES, INPUT_PINS, TEST_FILTER
...
def prepare(args):
    ...
    need(set(copied)==set(original),'Unexpected code source inventory')
    ...
    record={'kind':'attention64-staged-preparation-v1','status':'prepared_not_built',
...
def load(args):
    ...
    need(record['kind']=='attention64-staged-preparation-v1' and record['test_filter']==FILTER,'Wrong preparation')
...
def build(args):
    ...
    result={**start,'status':'built_not_executed','kind':'attention64-staged-build-v1','executables':executables,
...
def operators(args):
    ...
    need(build['kind']=='attention64-staged-build-v1' and build['preparation_sha256']==args.preparation_sha256 and build['status']=='built_not_executed','Wrong build')
    ...
    need(names==sorted(TEST_NAMES),'Wrong operator test inventory')
    # Keep every selected test's completion line intact for the execution gate.
    ...
    passed=code==0 and f'{len(TEST_NAMES)} passed; 0 failed' in text and all(text.splitlines().count(f'test {n} ... ok')==1 for n in names)
    operator_fixtures()
    closure(directory,record);need(sha(exe)==expected,'Test binary changed during execution')
    write(directory/'operators.json',{'kind':'attention64-staged-operators-v1','status':'passed' if passed else 'failed','exit_code':code,
        ...
        'selected_test_count':len(TEST_NAMES),'all_selected_tests_actually_passed':passed,
