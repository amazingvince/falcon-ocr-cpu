...
fn assert_cli_error(output: Output, expected: &str) {
    ...
    assert!(
        ...
        "CLI terminated without an exit code"
    );
    assert!(
...
fn public_runner_errors_and_empty_collections_do_not_generate() {
    ...
    let model = Arc::new(
        Model::load(&model_dir)
            .expect("install pinned assets before explicitly running this ignored test"),
    );
    error_contains(
        Runner::new(
            model.clone(),
    ...
    assert!(
        runner
            .recognize_batch(no_images, &options)
            .unwrap()
            .is_empty()
    );
    assert!(
    ...
    assert!(
        runner
            .recognize_files(no_files, &options)
            .unwrap()
            .is_empty()
    );
    for (label, invalid, expected) in invalid_options() {
        ...
        error_contains(
            runner.recognize_batch(std::slice::from_ref(&valid), &invalid),
            expected,
        );
        error_contains(
    ...
    for (width, height) in [(0, 0), (0, 8), (8, 0)] {
        ...
        error_contains(
            runner.recognize_batch_with_trace(
                &[valid.clone(), invalid],
                &options,
                &mut NoModelWork,
            ),
    ...
    for (label, path, expected) in &cases.invalid {
        ...
        error_contains(
            runner.recognize_files(&[&cases.valid, path], &options),
            expected,
        );
    }
}
...
fn cli_runtime_errors_exit_nonzero_without_partial_json_results() {
    ...
    let run = |arguments: &[&str], files: &[&Path]| {
        let mut command = cli();
        command.arg("--model").arg(model_directory()).args([
            "--threads",
            "1",
            "--backend",
            "scalar",
    ...
    assert_cli_error(
        run(&["--batch-size", "2"], &[&cases.valid, missing]),
        "reading image",
    );
