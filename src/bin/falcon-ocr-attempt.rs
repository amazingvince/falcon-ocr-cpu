...
use falcon_ocr::{Model,Runner,RunnerConfig,Backend,CacheLayout,WeightLayout,GenerationOptions,
    attempt::{Profile,Telemetry},trace::TensorTrace};
use sha2::{Digest,Sha256};
use std::{path::{Path,PathBuf},sync::Arc,time::Instant,io::{Read,Write}};

#[derive(Parser)]
#[command(about="Falcon-OCR v1.5 integrated CPU experiment (UNQUALIFIED)")]
struct Cli {
    #[arg(long,default_value="artifacts/model",global=true)] model:PathBuf,
    #[arg(long,value_enum,default_value="reference",global=true)] profile:Profile,
    #[arg(long,value_enum,default_value="auto",global=true)] backend:Backend,
    #[arg(long,default_value_t=16,global=true)] threads:usize,
    #[arg(long,default_value_t=1,global=true)] batch_size:usize,
    /// Custom W8G64 safetensors overlay, made by attempt3/convert_w8.py.
    #[arg(long,global=true)] w8_artifact:Option<PathBuf>,
...
enum Command {
    Doctor,
    Bench {
        #[arg(required=true)] images:Vec<PathBuf>,
        #[arg(long,default_value_t=4096)] max_new_tokens:usize,
        #[arg(long,default_value_t=1536)] max_dimension:u32,
        #[arg(long,default_value_t=64)] min_dimension:u32,
        #[arg(long,default_value_t=1)] warmup:usize,
        #[arg(long,default_value_t=3)] samples:usize,
    ...
    Trace {
        ...
        #[arg(long,default_value_t=4)] max_new_tokens:usize,
    },
}
fn digest(path:&Path)->Result<String> {
    let mut file=std::fs::File::open(path)?; let mut h=Sha256::new(); let mut buf=[0u8;65536];
...
fn write_new(path:&Path,value:&serde_json::Value)->Result<()> {
    ...
    serde_json::to_writer_pretty(&mut f,value)?;f.write_all(b"\n")?;Ok(())
}
fn main()->Result<()> {
    let args=Cli::parse();
    ...
    if matches!(args.command,Command::Doctor) {
        println!("{}",serde_json::to_string_pretty(&hardware)?);return Ok(());
    }
    // Fail invalid output/options before model allocation and any recognition.
    match &args.command {
        Command::Bench{samples,report,max_new_tokens,max_dimension,min_dimension,images,..} => {
            ensure!(*samples>0,"samples must be positive"); ensure!(!report.exists(),"report already exists");
            GenerationOptions{max_new_tokens:*max_new_tokens,min_dimension:*min_dimension,max_dimension:*max_dimension}.validate()?;
            for p in images {ensure!(p.is_file(),"missing image {}",p.display());}
        },
        Command::Trace{fixture,output,..} => {ensure!(fixture.is_file(),"missing fixture");ensure!(!output.exists() && !output.with_extension("json").exists(),"trace or sidecar already exists");},
        Command::Doctor => unreachable!(),
    }
    let started=Instant::now();
    let model=Arc::new(Model::load_attempt(&args.model,args.profile,args.w8_artifact.as_deref())?);
    let load_ms=started.elapsed().as_secs_f64()*1000.0;
    let memory=model.attempt_memory_report();
    let runner=Runner::new(model.clone(),&args.model,RunnerConfig{
        threads:args.threads,batch_size:args.batch_size,backend:args.backend,
        cache_layout:CacheLayout::Compact,weight_layout:WeightLayout::Unpacked,
    })?;
    eprintln!("profile={} load/import={:.1}ms; experimental quality is NOT qualified",args.profile.label(),load_ms);
    match args.command {
        Command::Bench{images,max_new_tokens,max_dimension,min_dimension,warmup,samples,report} => {
            let options=GenerationOptions{max_new_tokens,max_dimension,min_dimension};
            let inputs=images.iter().map(|p|Ok(serde_json::json!({"path":p,"sha256":digest(p)?})))
                .collect::<Result<Vec<_>>>()?;
            for _ in 0..warmup {runner.recognize_files(&images,&options)?;}
            let mut records=Vec::with_capacity(samples);
            for index in 0..samples {
                let mut telemetry=Telemetry::default();let t=Instant::now();
            ...
            println!("{}",serde_json::json!({"report":report,"profile":args.profile,"samples":samples}));
        },
        Command::Trace{fixture,output,max_new_tokens} => {
            let mut trace=TensorTrace::default();let result=runner.trace_reference(fixture,max_new_tokens,&mut trace)?;
            if let Some(parent)=output.parent().filter(|p| !p.as_os_str().is_empty()) {std::fs::create_dir_all(parent)?;}
            trace.save(&output)?;
            write_new(&output.with_extension("json"),&serde_json::json!({"profile":args.profile,"quality_qualified":false,"output":result,"memory_policy":memory}))?;
        },
        Command::Doctor=>unreachable!(),
    }
    Ok(())
