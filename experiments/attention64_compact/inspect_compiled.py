...
PDB=Path('D:/falcon-ocr-rust-builds/attention64-compact-v1/release/examples/ocr_bench.pdb')
PDBUTIL=Path('C:/Program Files/LLVM/bin/llvm-pdbutil.exe')
OBJDUMP=Path('C:/Program Files/LLVM/bin/llvm-objdump.exe')
OUT=BUILD/'compiled-compact-head'
NAME='falcon_ocr::kernels::attention64_candidate::compact_head'
BUILD_SHA='68b1667cb7fed0d5470ec8b9153e87dee7198787f70667e71f807be9f8cae7f0'
...
dot=[(a,m,op) for a,m,op in instructions if 0x1400b17a0<=a<=0x1400b1841]
axpy=[(a,m,op) for a,m,op in instructions if 0x1400b1a3b<=a<=0x1400b1ae8]
...
branch={a:(m,op) for a,m,op in instructions if 0x1400b186b<=a<=0x1400b18d9}
need(branch[0x1400b1872]==('subq','0x388(%rsp), %rax'),'Prefix-length comparison changed')
need(branch[0x1400b187a][0]=='jae' and branch[0x1400b187a][1].startswith('0x1400b18b0'),'Generated-key branch changed')
need(branch[0x1400b187c]==('imulq','%r13, %rcx') and branch[0x1400b18b0]==('imulq','%r9, %rax'),'Distinct prefix/generated strides changed')
need(branch[0x1400b18a5][1].startswith('0x1400b17a0') and branch[0x1400b18d9][1].startswith('0x1400b17a0'),'Branches no longer rejoin fixed dot')
save('prefix-generated-branch.txt','\n'.join(line for line in disassembly.splitlines() if any(line.startswith(f'{a:x}:') for a in branch))+'\n')
...
report={'kind':'attention64-compiled-compact-head-v1','status':'fixed_width_inline_confirmed',
    ...
    'prefix_generated_branch':{'compare_subtract':'0x1400b1872','unsigned_branch':'0x1400b187a',
        'prefix_address_path':'0x1400b187c..0x1400b18a5','generated_address_path':'0x1400b18b0..0x1400b18d9',
        'shared_dot_target':'0x1400b17a0','retained':True},
    'observations':['Dot is unrolled into eight YMM FMAs: four zero accumulations and four second products.',
        ...
        'Unsigned key-minus-prefix comparison selects prefix key*query_width+head*64 or generated (key-prefix)*kv_width+kv_head*64.',
        'Both address paths rejoin the same inline fixed64 dot; GQA V indexing uses kv_width and kv_head*64.',
