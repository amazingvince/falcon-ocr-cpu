"""Offline head-prefix protocol tests. All process and model-input work is mocked."""
import argparse
import copy
import hashlib
...
SPEC = importlib.util.spec_from_file_location("head_prefix_candidate_protocol", Path(__file__).with_name("benchmark_head_prefix_candidate.py"))
...
class HeadPrefixProtocolTests(unittest.TestCase):
    def test_explicit_cli_and_report_spellings_for_each_job(self):
        ...
        for name,cli,report in [('control-before','compact','compact'),
                                ('candidate','head-contiguous-prefix','head_contiguous_prefix'),
    ...
    def test_source_allowlist_exact_with_no_harness_change_or_deletion(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            mutations=[lambda c:c['source_sha256'].update({'src/unreviewed.rs':'6'*64}),
                       ...
                       lambda c:c['source_sha256'].pop('src/head_contiguous_prefix_model_tests.rs'),
    ...
    def protocol(self,folder):
        ...
        plan={'kind':bc.KIND,'output_directory':str(folder),'host_platform':platform.platform(),
              ...
              'qualification':{'path':str(folder/'model-run-v1/comparison.json'),'sha256':'e'*64}}
        files={str(Path(bc.__file__).resolve()),str(bc.ROOT/'research/benchmarks/scripts/realistic_benchmark.py'),
    ...
    def mocked_inputs(self,stack,workload,builds):
        ...
        folder=builds['candidate'][1].parent
        stack.enter_context(mock.patch.object(bc,'qualification_receipts',return_value=(
            {'path':str(folder/'model-run-v1/comparison.json'),'sha256':'e'*64},{})))
    ...
    def qualification_fixture(self, folder, mutate=None):
        """Only tiny JSON receipts and placeholder artifacts; no binaries/models."""
        folder=folder.resolve(); (folder/'benchmark-build').mkdir(); (folder/'model-run-v1').mkdir()
        bound={}
        def save(path,value):
            raw=json.dumps(value,sort_keys=True).encode();path.write_bytes(raw)
            digest=hashlib.sha256(raw).hexdigest();bound[str(path)]=digest
            return digest
        def blob(name):
            path=folder/name;path.write_text('synthetic')
            bound[str(path)]=hashlib.sha256(path.read_bytes()).hexdigest()
            return bound[str(path)]
        candidate={'source_sha256':{n:'a'*64 for n in (*bc.SOURCE_CHANGES,*bc.SOURCE_ADDITIONS)}}
        candidate_sha=save(folder/'benchmark-build/build.json',candidate)
        prep={'kind':'head-contiguous-prefix-preparation-v1','status':'prepared_not_built',
              'control_build_sha256':bc.CONTROL_SHA,'changed_control_sources':list(bc.SOURCE_CHANGES),
              'added_sources':list(bc.SOURCE_ADDITIONS),'selected_test_count':len(bc.OPERATOR_TESTS),
              'selected_tests':list(bc.OPERATOR_TESTS),'test_filters':list(bc.OPERATOR_FILTERS),
              'project_source_sha256':candidate['source_sha256'].copy()}
        build={'kind':'head-contiguous-prefix-build-v1','status':'built_not_executed',
               'benchmark_build_sha256':candidate_sha,'executables':{
                   label:{'path':label+'.exe','sha256':blob(label+'.exe')}
                   for label in ('operators','qualification')}}
        groups=[]
        for index,selected in enumerate(bc.OPERATOR_FILTERS):
            log=f'operator-{index}.log'
            groups.append({'filter':selected,'tests':sorted(n for n in bc.OPERATOR_TESTS if n.startswith(selected)),
                           'passed':True,'exit_code':0,'log':log,'log_sha256':blob(log)})
        ops={'status':'passed','source_closure':True,'all_selected_tests_actually_passed':True,
             'full_model_or_benchmark_executed':False,'tests':sorted(bc.OPERATOR_TESTS),
             'selected_test_count':len(bc.OPERATOR_TESTS),'groups':groups,
             'binary_sha256':build['executables']['operators']['sha256']}
        comparison={'kind':'head-contiguous-prefix-model-comparison-v1','status':'passed',
                    'source_input_closure':True,'performance_measurement':False,
                    'runtime':bc.QUALIFICATION_RUNTIME.copy(),'canonical_cpu_tensors_exact':1904,
                    'mixed_tensors_exact':2144,'actual_teacher_argmax_exact':17,
                    'independent_eos_counts':[17,2,6],'fresh_processes':2,'warmed_zero_allocation_intervals':4,
                    'jobs':[{'layout':layout,'exit_code':0,'owned_process_reaped':True,
                             'binary_sha256':build['executables']['qualification']['sha256']}
                            for layout in ('compact','head_contiguous_prefix')]}
        for layout in ('compact','head_contiguous_prefix'):blob('model-run-v1/'+layout+'.json')
        if mutate:mutate(prep,build,ops,comparison)
        prep_sha=save(folder/'preparation.json',prep)
        build['preparation_sha256']=prep_sha
        build_sha=save(folder/'build.json',build)
        ops.update(preparation_sha256=prep_sha,build_sha256=build_sha)
        ops_sha=save(folder/'operators.json',ops)
        comparison.update(preparation_sha256=prep_sha,build_sha256=build_sha,operators_sha256=ops_sha,
                          bound_sha256=bound.copy())
        path=folder/'model-run-v1/comparison.json';digest=save(path,comparison)
        return folder/'benchmark-build/build.json',path,digest

    def test_accepted_qualification_joins_source_build_ops_and_model_receipts(self):
        with tempfile.TemporaryDirectory() as temp:
            args=self.qualification_fixture(Path(temp))
            proof,bound=bc.qualification_receipts(*args)
            self.assertEqual(proof['path'],str(args[1]))
            self.assertEqual(proof['sha256'],args[2])
            self.assertEqual(len(bound),11)
            for name in ('preparation.json','build.json','operators.json','operators.exe','qualification.exe',
                         'model-run-v1/compact.json','model-run-v1/head_contiguous_prefix.json'):
                self.assertIn(str(Path(temp).resolve()/name),bound)

    def test_rejects_unqualified_or_different_source_build_results(self):
        mutations=[lambda p,b,o,c:c.update(status='failed'),
                   lambda p,b,o,c:c.update(source_input_closure=False),
                   lambda p,b,o,c:c.update(canonical_cpu_tensors_exact=1903),
                   lambda p,b,o,c:c.update(warmed_zero_allocation_intervals=3),
                   lambda p,b,o,c:c['jobs'][1].update(exit_code=1),
                   lambda p,b,o,c:c['jobs'][1].update(binary_sha256='b'*64),
                   lambda p,b,o,c:c['runtime'].update(threads=8),
                   lambda p,b,o,c:p['selected_tests'].pop(),
                   lambda p,b,o,c:p['project_source_sha256'].update({'src/model.rs':'b'*64}),
                   lambda p,b,o,c:b.update(benchmark_build_sha256='c'*64),
                   lambda p,b,o,c:o.update(status='failed'),
                   lambda p,b,o,c:o['groups'][0].update(passed=False),
                   lambda p,b,o,c:o['groups'][0]['tests'].pop()]
        for change in mutations:
            with self.subTest(change=change), tempfile.TemporaryDirectory() as temp:
                args=self.qualification_fixture(Path(temp),change)
                with self.assertRaises(ValueError):bc.qualification_receipts(*args)

    def test_receipt_hash_lineage_and_missing_raw_results_fail_closed(self):
        for kind in ('digest','ancestry','raw-result','path'):
            with self.subTest(kind=kind), tempfile.TemporaryDirectory() as temp:
                candidate,path,digest=self.qualification_fixture(Path(temp))
                if kind=='digest':digest='0'*64
                elif kind=='ancestry':(Path(temp)/'operators.json').write_text('{}')
                elif kind=='path':path=Path(temp)/'different/comparison.json'
                else:
                    value=json.loads(path.read_bytes())
                    value['bound_sha256'].pop(str(Path(temp).resolve()/'model-run-v1/head_contiguous_prefix.json'))
                    digest=self.save(path,value)
                with self.assertRaises(ValueError):bc.qualification_receipts(candidate,path,digest)
