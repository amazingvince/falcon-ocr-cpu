...
def main():
    ...
    lookup = ROOT / "experiments/rms_norm/rsqrt_lookup.rs"
    require(sha(lookup) == proof["source_sha256"]["experiments/rms_norm/rsqrt_lookup.rs"],
            "lookup source differs from the exhaustively validated capture")
    controls = [ROOT / "scripts/build_windows.ps1", ROOT / "scripts/compare_traces.py",
                Path(__file__).resolve(), ROOT / "experiments/rms_norm/capture_width768_intervention.py", lookup]
